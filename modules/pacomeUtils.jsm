...
const gParams = {
  ...
  // valeur par défaut paramétrage sécurisé ?
  PACOME_URLPARAM_AUTH: false,
  // pref associée de paramétrage sécurisé
  PREF_URLPARAM_AUTH: "pacome.urlparam.auth",

  // préférences kerberos
  PREF_KRB_ENABLED: "pacome.krbauth.enabled",
  // préférence user d'ignorer kerberos
...
function PacomeStoreParamsCreds(url, creds) {
  ...
  // on retire les anciens password enregistrés (si avance/retour dans l'assistant)
...
/**
 * Renvoie le(s) {username, password} stocké(s) dans le passwordManager pour l'url fournie
...
function PacomeFetchParamsCreds(url, first=true) {
  ...
  if (first)  {
    if (logins.length === 0) {
      // on renvoie un login vide plutôt que null
...
/**
 * Renvoie l'url de récupération des paramétrages, en fonction de la configuration et des choix de l'utilisateur (kerberos, authentification...)
 * @returns string
 */
